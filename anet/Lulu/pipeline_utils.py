...
num_frames = 32  # global: number of frames extracted for encoder 
...
def init_trans(path):
        train_id_path = os.path.join(path, "new_train_translate.json")
        test_id_path = os.path.join(path, "new_test_translate.json")
        train_ids = json.load(open(train_id_path,'r'))[0]
        test_ids = json.load(open(test_id_path,'r'))[0]
        vocab_path = os.path.join(path, "new_vocab_translate.json")
...
def translate_gts(gts_qids, train_ids, test_ids, max_len,flag="train"):
        B = len(gts_qids)
        ...
        if flag=="train":
                ...
                for qid in gts_qids:
                        v = train_ids[qid][:max_len] 
                        outs[gts_qids.index(qid)][:len(v)] = torch.tensor(v) 
        else:
                for qid in gts_qids:
                        v = test_ids[qid][:max_len]  
                        outs[gts_qids.index(qid)][:len(v)] = torch.tensor(v) 
        return outs 
        ...
        for _id, _ in enumerate(idx_ranges):
                ...
                try:
                        ...
                        if _[0] < 128 and _[-1] >= 128:
                                ...
                                out_feats = torch.zeros((num_frames,h))
