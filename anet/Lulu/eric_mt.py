...
class SoftDotAttention(nn.Module):
	...
	def forward(self, context, h, mask=None):
		...
		#print("linear shape ", self.linear_in.shape)
		...
		if mask is not None:
			...
			attn.data.masked_fill_(mask.cuda(), -float('inf'))
		attn = self.sm(attn)
...
class Encoder(nn.Module):
	def __init__(self, vocab_size, embed_size, hidden_size,
		...
		self.src_embed = nn.Embedding(vocab_size, embed_size) # input srccap: ids 
		self.src_encoder = nn.LSTM(input_size=embed_size, hidden_size=hidden_size // 2, num_layers=n_layers,
									 dropout=dropout, batch_first=True, bidirectional=True)

		self.frame_embed = nn.Linear(500, self.embed_size)
...
class Decoder(nn.Module):
	def __init__(self, embed_size, hidden_size, vocab_size,
		...
		self.vid_attention = SoftDotAttention(embed_size, 2*hidden_size)

		#self.src_attention = SoftDotAttention(embed_size, 2*hidden_size)
		#self.vid_attention = SoftDotAttention(embed_size, 2*hidden_size)

		self.decoder = nn.LSTM(embed_size*3, 2*hidden_size,
						  n_layers, dropout=dropout, batch_first=True)

		self.fc = nn.Sequential(nn.Linear(2*self.hidden_size, self.embed_size),
								   nn.Tanh(),
	...
	def onestep(self, input, last_hidden, src_out, vid_out, src_mask):
		...
		output, hidden = self.decoder(rnn_input.contiguous(), last_hidden) 
