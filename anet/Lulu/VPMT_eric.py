...
#padding_idx = 
#charades_eric_32f_200_vpmt.pkl
...
class VPMT(nn.Module):
	...
	def __init__(self, arg, dataset, optim_params_type="All"):
		...
		self.decoder = Decoder(1000, hidden_size=arg.text_embed_size, vocab_size=dec_vocab_size)
	...
	def compute_loss_nll(self, scores, targets):
		...
		for _ in range(lens):
			pred = scores[:, _, :]
			gold = targets[:, _]
			#vpmt_pip.dec_criterion(pred, gold)
			if _ == 0:
				self.nllloss = self.dec_criterion(pred, gold)
			else:
				self.nllloss += self.dec_criterion(pred, gold)
	...
	def update_lr(self):
		if self.optim_params_type == "All":
			...
			print("========= UPDATE LR RATE AT {} =========".format(cur_lr*0.1))
		else:
			cur_lr = self.enc_optimizer.param_groups[0]['lr']
			self.enc_optimizer.param_groups[0]['lr']= cur_lr * 0.1 
			self.dec_optimizer.param_groups[0]['lr']= cur_lr * 0.1 
			self.lgi_optimizer.param_groups[0]['lr']= cur_lr * 0.1 
			print("========= UPDATE LR RATE AT {} =========".format(cur_lr*0.1))

	def select_optimizer_type(self, model_params):
		...
		if opt_type == "SGD":
			...
			optimizer = torch.optim.Adam(
				model_params, lr=lr, betas=betas,
				weight_decay=weight_decay)
		elif opt_type == "Adadelta":
			optimizer = torch.optim.Adadelta(model_params, lr=lr)
	...
	def create_optimizer(self):
		if self.optim_params_type == "All":
			model_params = self.LGI_params + self.encoder_params + self.decoder_params
			self.optimizer, self.scheduler = self.select_optimizer_type(model_params) 
		else:
			self.enc_optimizer, self.enc_scheduler = self.select_optimizer_type(self.encoder_params)
			self.dec_optimizer, self.dec_scheduler = self.select_optimizer_type(self.decoder_params)
			self.lgi_optimizer, self.lgi_scheduler = self.select_optimizer_type(self.LGI_params)
	...
	def update(self):
		...
		if self.optim_params_type == "All":
			...
			if self.lgi_optimizer == None:
				self.create_optimizer() 
				self.lgi_optimizer.zero_grad()
				self.enc_optimizer.zero_grad()
				self.dec_optimizer.zero_grad()

			self.total_loss.backward()
			if self.enc_scheduler is not None: self.enc_scheduler.step()
			if self.dec_scheduler is not None: self.dec_scheduler.step()
			if self.lgi_scheduler is not None: self.lgi_scheduler.step()
			self.lgi_optimizer.step()
	...
	def forward(self, net_inps, gts, prefix="", mode="Train"):
		...
		if self.simplified_trans:
			...
			self.scores = self.decoder(net_inps['query_labels'].long(), gts_queries.long(), init_hidden, src_out, vid_out, 10, teacher_forcing_ratio=0.2)
			self.compute_loss_nll(self.scores, gts_queries.long())

		else:
			self.scores = self.decoder(net_inps['query_labels'], gts['query_labels'], init_hidden, src_out, vid_out, 10, teacher_forcing_ratio=0.2)
	...
	def print_info_but_lgi(self, mode, epoch, _iter, logger=None):
		...
		txt += ", TOTAL LOSS = {:.4f}, LGI LOSS = {:.4F},  NLL LOSS = {:.4f}".format(self.total_loss, self.total_loss.item() - self.nllloss.item(), self.nllloss.item())
		if logger:
			logger.info(txt)
		else:
			print(txt)
...
if __name__ == "__main__":
	...
	vis_data = D.get_samples(2)
	net_inps, gts = vpmt_pip.LGI_model.prepare_batch_w_pipline_anet(vis_data, True)
	lgi_out = vpmt_pip.LGI_model(net_inps) 
	durations = torch.tensor(gts['duration']).unsqueeze(-1).cuda()
	v_feats = extract_frames(lgi_out['grounding_loc'], net_inps['video_feats'], durations)
	src_out, init_hidden, vid_out = vpmt_pip.forward_vse_emb(v_feats, net_inps['query_labels'], net_inps['query_lengths'])
	gts_queries = translate_gts(gts['qids'], vpmt_pip.train_ids, vpmt_pip.test_ids, 10)
	#vpmt_pip.decoder.beam_decoding(net_inps['query_labels'].unsqueeze(1), init_hidden, src_out, vid_out, 10)
	vpmt_pip.scores = vpmt_pip.decoder(net_inps['query_labels'].long(), gts_queries.long(), init_hidden, src_out, vid_out, 10, teacher_forcing_ratio=0.2)
	vpmt_pip.compute_loss_nll(vpmt_pip.scores, gts_queries.long())
	outputs, pred_lengths = vpmt_pip.decoder.inference(net_inps['query_labels'].long(), gts_queries.long(), init_hidden, src_out, vid_out, 10, teacher_forcing_ratio=0.2)
	#src, init_hidden, src_out, vid_out, max_len, 
	seq, pred_lengths = vpmt_pip.decoder.beam_decoding(net_inps['query_labels'].long(), init_hidden, src_out, vid_out, 10)
	#seq, pred_lengths = vpmt_pip.decoder.beam_decoding(net_inps['query_labels'].long(), gts_queries.long(), init_hidden, src_out, vid_out, 10, teacher_forcing_ratio=0.2)
