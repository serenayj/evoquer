...
class SupervisedTrainer(object):
	...
	def __init__(self, full_cfg, lgi_config, config, train_db, test_db, dataset, pretrained_lgi_path=None):
		...
		self.batch_size = 32
		self.lr_update_every = 90
		self.epoch = 200 
		self.save_loss = 1000
		if pretrained_lgi_path:
			self.model.LGI_model.load_state_dict(torch.load(pretrained_lgi_path))
			self.prefix = today + dataset + "_eric_32f_"+str(self.epoch)+"ep_pre_lgi"+"_"+self.optim_types
		else:
			self.prefix = today + dataset + "_eric_32f_"+str(self.epoch)+"_"+self.optim_types
...
if __name__ == "__main__":
	...
	#pretrained_lgi_path = "charades_ep120_lgionly__vpmt_only.pkl" 
