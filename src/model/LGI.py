...
class LGI(AbstractNetwork):
        def __init__(self, config, vocab_size, logger=None, verbose=True):
        ...
        def _build_network(self):
                ...
                self.dataset = self.config["misc"]["dataset"]

                mconfig['query_enc_emb_idim'] = self.vocab_size 
        ...
        def _infer(self, net_inps, mode="forward", gts=None):
                # fetch inputs
                word_labels = net_inps["query_labels"].to(self.device) # [B,L] (nword == L)
                word_masks = net_inps["query_masks"].to(self.device) # [B,L]
                c3d_feats = net_inps["video_feats"].to(self.device)  # [B,T,d_v]
                print("c3d_feats shape", c3d_feats.shape)
                if len(net_inps["video_masks"].shape) == 3:
                        seg_masks = net_inps["video_masks"].squeeze(2).to(self.device) # [B,T]
                else:
                        seg_masks = net_inps["video_masks"].to(self.device)
        ...
        def prepare_batch_w_pipline(self,batch, gpu_flag):
                ...
                gt_inps_list = ["grounding_start_pos", "grounding_end_pos",
                                   "grounding_att_masks"]
                net_inps, gts = {}, {}

                ### Different dataset loader 
                ...
                for k in tensor_inps_keys:
                        ...
                        if torch.is_tensor(val[0]):
                                ...
                                if gpu_flag:
                                        #print("moving to cuda ", k)
                                        net_inps[k].to(self.device)
                                        gts[k].to(self.device) 
                ...
                for k in gt_inps_list:
                        gts[k] = torch.tensor(gts[k]).to(self.device)
                return net_inps, gts 

        def prepare_batch_w_pipline_anet(self,batch, gpu_flag):
                #print("GPU FLAG ",gpu_flag)
                input_keys = ['vids', 'qids', 'timestamps', 'duration', 'query_lengths', 'query_labels', 'query_masks', 'grounding_start_pos', 'grounding_end_pos', 'grounding_att_masks', 'nfeats', 'video_feats', 'video_masks']
                net_inps_keys = ['query_lengths', 'query_labels', 'query_masks', 'grounding_att_masks', 'video_feats', 'video_masks']
                gt_list = ["vids", "qids", "timestamps", "duration",
                                   "grounding_start_pos", "grounding_end_pos",
                                   "grounding_att_masks", "nfeats"]
                both_list = ["grounding_att_masks"]
                tensor_inps_keys = ['query_labels', 'query_masks', 'grounding_att_masks', 'video_feats', 'video_masks']
                gt_inps_list = ["grounding_start_pos", "grounding_end_pos",
                                   "grounding_att_masks"]
                net_inps, gts = {}, {}
                for items in batch:
                ...
                for k in tensor_inps_keys:
                        ...
                        if torch.is_tensor(val):
                                if gpu_flag:
                                        net_inps[k].to(self.device)
                                        if k in gts: gts[k].to(self.device)
                        else:
                                if k == "description_labels":
                                        val = torch.tensor(net_inps[k])
                                        net_inps[k] = val
                                        if gpu_flag:
                                                net_inps[k].to(self.device)
                                                if k in gts: gts[k].to(self.device)
                                        ...
                                        tmp = torch.stack(val)
                                        if k == "query_labels": tmp = tmp.squeeze(1)
                                        if k == "query_masks": tmp = tmp.squeeze(1)
                                        net_inps[k] = tmp 
                                        if k in gts: gts[k] = tmp 
                                        if gpu_flag:
                                                net_inps[k].to(self.device)
                                                if k in gts: gts[k].to(self.device)
