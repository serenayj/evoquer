...
def create_logger(config, logger_name, log_path):
    #if os.path.exists(config["misc"]["result_dir"])
...
def test(config, loader, net, epoch, eval_logger=None, mode="Test"):
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(loader, desc="{}".format(mode)):
            # forward the network
            try:
                net_inps, gts = net.prepare_batch_w_pipline(batch, True)
            except:
                net_inps, gts = prepare_batch_w_pipline_anet(batch, True)
            outputs = net.forward_only(net_inps) # only forward
            #print("counter test ", ii) 
