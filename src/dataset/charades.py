...
class DescriptionBuilder(object):
	"""DescriptionBuilder: get all descriptions from annotation"""
	def __init__(self):
		super(DescriptionBuilder, self).__init__()
		self.vocab = set() 
		self.desp = {}
		#self.get_description(split)
		#self.get_description(split)
	def get_description(self, split):	 
		words = [] 
		if split == "train":
			path = "data/charades/annotations/Charades_v1_train.csv"
		else:
			path = "data/charades/annotations/Charades_v1_test.csv"

		with open(path,"r") as f:
			rea = csv.reader(f)
			for line in rea:
				vid = line[0]
				self.desp[vid] = [line[6].lower().split()]
				words.extend(line[6].lower().split())
				if line[8] == '{}' or split == "test":
					continue 
				else:
					desps = line[8].split(";")
					for d in desps:
						self.desp[vid].append(d.lower().split())
						words.extend(d.lower().split())
		self.vocab.update(set(words)) # update vocabulary 

	def encoded_description(self, vid,wtoi):
		#print("current wtoi length ", len(wtoi))
		max_length = 40
		tokens_lst = self.desp[vid]
		lengths,labels = [] , [] 
		# obtain query labels and their lengths
		for tokens in tokens_lst:
			lengths.append(min(len(tokens), max_length))
			#labels[vid].append(np.zeros((max_length), dtype=np.int))
			tmp = np.zeros((max_length), dtype=np.int)
			# words -> labels
			for wi,w in enumerate(tokens):
				if wi == max_length: break
				tmp[wi] = wtoi.get(w, 1)   # 1: <UNK> token
			labels.append(tmp)

		encoded = {
			"descrip_lengths": lengths,
			"descrip_labels": labels,
		}
		return encoded
...
class CharadesDataset(AbstractDataset):
	def __init__(self, config):
		...
		if self.feature_type == "I3D":
			self.feat_path = config.get(
				"video_feature_path",
				"data/charades/i3d_finetuned/{}.npy"
			)
		else:
			raise ValueError("Wrong feature_type")

		# Add descriptions from train and test files 
		self.descriploader = DescriptionBuilder() 
		...
		"""
		If already built descp vocab, just get the data, whether it is train or test  
		"""
		#self.descriploader.get_description(self.split)
	...
	def __getitem__(self, idx):
		...
		#print("D LABEL LST ", d_label_lsts)

		return_instance = []  

		for _d in range(len(d_label_lsts['descrip_lengths'])):
			description_length = d_label_lsts['descrip_lengths'][_d]
			description_labels = d_label_lsts["descrip_labels"][_d]
			...
			return_instance.append(instance)
	...
	def collate_fn(self, item):
		...
		#print("======== INSIDE COLLATE FN, DATA========", type(data[0][0]))
		#print(data[0][0].keys())
		#self.batch = data 
		try:
			...
			return batch
		except Exception as e: # BAD BATCH 
			print(e) 
	...
	def _get_data_path(self, config):
		...
		description_info_path = os.path.join(root_dir,
				"description_info", "{}_description_info_F{}_L{}_{}.json".format(split, F, L, FT))
		...
		io_utils.check_and_create_dir(os.path.join(root_dir, "description_info"))

		self.paths = {
			"grounding_info": grounding_info_path,
			"query_labels": query_label_path,
			"query_info": query_info_path,
			'description_info': description_info_path,
		}
		return self.paths
	...
	def generate_labels(self, config):
		...
		self.descriploader.get_description("train")
		self.descriploader.get_description("test") 
		#io_utils.write_json(self.paths["description_info"], self.descriploader.desp) 
		print("YANJUN BOT: SUCCESSFULLY BUILT ======> DESCRIPTION LOADER ")
		

		""" Query information """
		if not os.path.exists(self.paths["query_labels"]):
			print("IF NOT EXIST QUERY LABELS ")
			...
			"""
			Vocabulary now has the desciprtions info  
			"""

			wtoi = self._build_vocab(train_anns, self.descriploader.vocab)
			self.wtoi = wtoi
			
			itow = {v:k for k,v in wtoi.items()}
			self.itow = itow
			print("YANJUN BOT: CURRENT VOCAB SIZE AFTER GENRATE LABEL", len(wtoi))
		...
		# 	descriptions_keys.append(ann['video_id'])

		# for vid in descriptions_keys:
		# 	self.descriploader.encoded_description(vid)
