"""
Modified for monolingual translation 
"""
...
class AbstractDataset(Dataset):
	...
	def _build_vocab(self, anns, descript_vocab, frequency_threshold=1):
		...
		if descript_vocab:
			# update vocabulary with description vocab if using descriptions 
			words.extend(list(descript_vocab))
			after = len(set(words)) 

			print("Both queries and descriptions vocabulary, Before {}\n With {} new vocabs from descriptions".format(before, after))
		self.words = words
	...
	def _encode_descrip(self, description, wtoi):
		labels, lengths = {}, {}
		for qid,ann in tqdm(anns.items(), desc="Encoding query"):
			tokens = ann["tokens"]
