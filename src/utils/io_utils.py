...
root_path = "/Users/yanjungao/Desktop/LGI4temporalgrounding-master/"
#root_path = ""
...
""" pickle helpers """
def load_pkl(file_path,root_path=None,verbose=True):
...
def write_pkl(file_path,root_path=None,verbose=True):
...
def write_yaml(file_path,root_path=None,verbose=True):
...
""" Text file helpers """
def append_text_to_file(file_path, append,root_path=None):
...
def load_lines_from(file_path,root_path=None):
    lines = []
    f = open(file_path, "r")
    while True:
        line = f.readline()
...
def load_lines_as_dict(file_path,root_path=None):
...
""" csv helpers """
def load_csv(file_path,root_path=None):
    out = []
...
def write_json(file_path, file_data,root_path=None,verbose=True):
    def default(o):
...
def load_json(file_path, root_path=None,verbose=True):
    if verbose:
...
def load_hdf5_as_numpy_array_dict(file_path, target_group=None):
    f = h5py.File(root_path + file_path, "r")
