...
class VPMT(nn.Module):
	...
	def __init__(self, arg, dataset):
		...
		if self.simplified_trans:
			if self.dataset == "anet":
				...
				dec_vocab_size = len(self.idx_vocab)
			self.itow = itow = {int(k): v for k,v in bot.model.idx_vocab.items()}  
		else:
			dec_vocab_size = self.vocab_size
			self.itow = {v:k for k,v in test_D.wtoi.items()} 

		if self.arg.use_attn:
			self.decoder = DecoderRNN(dec_vocab_size, arg.max_len, arg.text_embed_size,
	...
	def compute_loss_nll(self, decoder_outputs, target_variable):
		...
		"""
		self.nllloss.reset() 
		#print("target var ", target_variable.shape)
	...
	def combine_loss(self):
		if self.weight_loss:
			a, b = 1, 1 
			self.total_loss = a*self.nllloss.loss + b*self.lgi_loss['total_loss']
		else:
			self.total_loss = self.nllloss.loss + self.lgi_loss['total_loss'] 
	...
	def forward_encode(self, descriptions, lengths, video_emb, volatile=False):
		...
		hidden_out = torch.tanh(hidden_out) # Yanjun: Added activation function for embedding 
	...
	def forward(self, net_inps, gts, mode="Train"):
		...
		if self.no_desp:
			...
			decode_out = self.forward_decode(None, v_emb, sent_out)
		else:
			if self.arg.cuda:
		...
		if self.simplified_trans:
			...
			self.compute_loss_nll(decode_out[0], gts_queries.long())
		else:
		...
		if mode != "Train":
			prefix = "valid_"
			self.translate(self.itow, gts, prefix,write=True) 
